...
3. **Fetch the URL** (e.g. with `mcp_web_fetch` or equivalent). If the fetch fails (timeout, 404, unreachable), **do not add** the resource; inform the user and stop.
4. **From the fetched content**, determine:
   ...
   - **Category**: map to one of the existing categories using the list below. If nothing fits, **do not create a category yet**; ask the user to confirm the new category name and that it should be created.
